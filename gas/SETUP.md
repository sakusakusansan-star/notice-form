# Discord 連携のセットアップ

お知らせを投稿すると Discord に自動で流れ、日程の**前日**になるとアラートが飛ぶようにします。
作業は Google Apps Script（GAS）側だけです。このリポジトリの HTML はそのままで動きます。

---

## ⚠️ 最初に：Webhook URL は絶対にここに書かない

Discord の Webhook URL は「これを知っている人は誰でもそのチャンネルに投稿できる」鍵です。
このリポジトリは GitHub Pages で公開されているので、`index.html` やコードに直接書くと
URL が全世界に見えてしまいます。

そのため URL は **GAS のスクリプトプロパティ**（外から見えない保管場所）に入れます。

---

## 既存コードとの共存について

Apps Script は**全ファイルが同じグローバル空間を共有**します。同じ名前の変数や関数を
別ファイルで宣言すると後勝ちで上書きされ、既存の `doGet` / `doPost` が壊れます。

そのためこの2ファイルが定義する名前は、**すべて `ND_` / `nd` で始めています**。
既存コードの名前を上書きすることはありません。

シートの特定も既存の設定に乗ります。

1. `ND_SHEET_ID`（このファイルの設定。既定は空）
2. 既存コードの `SHEET_ID` を**読み取って**使う ← 通常はこれ
3. `getActiveSpreadsheet()`

3 はスタンドアロンのプロジェクトや時間主導トリガーからは `null` になるため、
既存コードが `SHEET_ID` を持っているならそれを使います（読むだけで、書き換えません）。
シート名も同様に `ND_SHEET_NAME` → 既存の `SHEET_NAME` → 先頭シート の順で解決します。

貼り付ける前に **`ndCheckSetup`** を実行すると、どのスプレッドシートのどのシートを見に行くか、
何件読めたかをログに出せます（Discord には何も送りません）。

---

## 手順（3ステップ）

既存のコードは一切書き換えません。ファイルを2つ足すだけです。

### 1. スクリプトを2つ追加する

1. お知らせを保存しているスプレッドシートを開く
2. メニューの **拡張機能 → Apps Script**
3. 左サイドバー「ファイル」の **＋ → スクリプト** でファイルを作り、名前を `NoticeDiscord` にする
4. [`NoticeDiscord.gs`](NoticeDiscord.gs) の中身を全部貼り付ける
5. 同じ手順でもう1つ `NoticeWatch` を作り、[`NoticeWatch.gs`](NoticeWatch.gs) の中身を貼り付ける
6. 保存（💾）

### 2. Webhook URL を登録する

1. 左サイドバー下の **⚙️ プロジェクトの設定**
2. 一番下の **スクリプト プロパティ** → **スクリプト プロパティを追加**
3. 次のように入力して保存

   - プロパティ： `DISCORD_WEBHOOK_URL`
   - 値： Discord で発行した Webhook URL（`https://discord.com/api/webhooks/...`）

> Webhook URL の作り方：Discord で投稿先チャンネルの
> **⚙️ → 連携サービス → ウェブフック → 新しいウェブフック** → 「ウェブフック URL をコピー」

### 3. トリガーを作る

1. エディタ上部の関数プルダウンで **`ndSetupTriggers`** を選ぶ
2. **実行** を押す
3. 初回は「承認が必要です」と出るので、アカウントを選んで許可する
   （「このアプリは確認されていません」と出たら **詳細 → （安全ではないページ）に移動 → 許可**）

これで完了です。

---

## どう動くか

| いつ | Discord に届く | `@here` で鳴る |
|------|---------------|---------------|
| 新規投稿（種別を問わず） | ✅ 1分以内 | ✅ |
| 編集（更新） | ✅ 1分以内 | ❌ |
| 削除 | ✅ 1分以内 | ❌ |
| 日程（`when`）の前日 朝9:00 | ✅ | ✅ |

**「届く」と「鳴る」は別物です。** Discord のサーバー通知設定を「@メンションのみ」に
している人には、`@here` が付いた通知しかプッシュが飛びません。

新規投稿で鳴らしたくない種別がある場合は `ND_MENTION_TYPES` を調整してください
（例：重要だけ鳴らすなら `['重要']`）。

`@here` はオンラインの人だけに飛びます。オフラインの人にも飛ばしたい場合は
`ND_ALERT_MENTION` を `'@everyone'` にしてください。

`ndSetupTriggers` を実行した時点で存在するお知らせは「既知」として記録されるので、
過去分が一斉に流れることはありません。

---

## 仕組み（2つの方式）

**A. 監視方式（既定・おすすめ）**
`NoticeWatch.gs` が1分ごとにシートを見て、増えた・変わった・消えたお知らせを通知します。
**既存の `doPost` を書き換える必要がありません。** 上の手順どおりならこちらで動きます。

**B. 即時方式（任意）**
既存の `doPost` の保存成功直後に、次の1行を足す方式です。1分待たずに即座に飛びます。

```js
ndNotify('create', {
  id: id, name: name, type: type, text: text,
  when: when, time: time, expire: expire
});
// 更新なら 'update'、削除なら 'delete'
```

A と B を両方使っても**二重通知にはなりません**。`ndNotify()` は送信した内容を
監視側の記録に反映するので、監視側は「もう送った分」として扱います。

### バックエンドをこれから作る場合

`doGet` / `doPost` がまだ無いなら、[`Code.gs`](Code.gs) も同じ手順で追加してください。
投稿・更新・削除・Discord 通知がひと通り入っています。

シートの1行目（見出し）はこの並びを想定しています。空のシートなら初回投稿時に自動で作られます。

| id | name | type | text | when | time | expire | created |
|----|------|------|------|------|------|--------|---------|

> 見出しは日本語（`投稿者` `種別` `内容` `日程` `時間` `表示期限`）でも認識します。
> 列の順番が違っても大丈夫です。`id` 列が無い場合は内容から自動で識別します。

`Code.gs` を入れた場合は、反映のために再デプロイが必要です。
**デプロイ → デプロイを管理 → ✏️ → バージョン「新バージョン」→ デプロイ**
URL は変わらないので `index.html` の `NOTICE_API` はそのままで大丈夫です。

---

## 動作確認

エディタの関数プルダウンから手動実行できます。

| 関数 | 内容 |
|------|------|
| `ndCheckSetup` | どのシートを見に行くかを確認（Discord には送らない） |
| `ndTestDiscord` | テスト投稿を1件 Discord に送る |
| `ndTestDailyAlert` | 明日が日程のお知らせに対して、前日アラートを今すぐ送る |
| `ndWatchNotices` | 監視を今すぐ1回走らせる |
| `ndResetWatch` | 監視の記録をリセット（次回実行時に現状を記録し直す） |

届かないときは、エディタ左の **実行数** からログを確認してください。
`スクリプトプロパティ DISCORD_WEBHOOK_URL が未設定` と出ていれば手順2をやり直しです。

---

## 設定を変えたいとき

各ファイルの先頭にまとまっています。変更後は `ndSetupTriggers` を再実行してください。

| 変数 | ファイル | 既定値 | 内容 |
|------|---------|--------|------|
| `ND_ALERT_HOUR` | NoticeDiscord.gs | `9` | 前日アラートを送る時刻（0〜23） |
| `ND_ALERT_MENTION` | NoticeDiscord.gs | `'@here'` | アラート時のメンション。鳴らしたくなければ `''` |
| `ND_MENTION_TYPES` | NoticeDiscord.gs | `['*']` | 新規投稿でメンションする種別。`['*']`=全種別、`['重要']`=重要だけ、`[]`=鳴らさない |
| `ND_MENTION_ON_UPDATE` | NoticeDiscord.gs | `false` | 更新（編集）でもメンションするか |
| `ND_SHEET_NAME` | NoticeDiscord.gs | `''` | 対象シート名。空なら先頭シート |
| `ND_WATCH_INTERVAL_MINUTES` | NoticeWatch.gs | `1` | 監視の間隔（1/5/10/15/30 分）。実行時間の上限に当たるなら 5 に |
| `ND_WATCH_BURST_LIMIT` | NoticeWatch.gs | `8` | これを超える変化はまとめて1通にする |
